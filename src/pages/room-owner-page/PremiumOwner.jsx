...
import { premiumOwnerFeatures, premiumOwnerPlans } from "../../data/roomsDekhoData";
    ...
    <div className="premium-container owner-premium">
      ...
      {isPremiumUser?.premium && (
        <div className="premium-active-banner">
          ...
          {new Date(status.endDate).toDateString()}
        </div>
      )}
      <MyPlans />
      ...
      <div className="compare-table">
        ...
        <table>
          ...
          <tbody>
            <tr><td>Rooms Allowed</td><td>2 Rooms Only</td><td className="premium-col">Unlimited Rooms</td></tr>
            <tr><td>Search Ranking</td><td>Last in results</td><td className="premium-col">Top Results</td></tr>
            <tr><td>Homepage Visibility</td><td>Hidden</td><td className="premium-col">Featured Banner</td></tr>
            <tr><td>Verified Badge</td><td>Not Available</td><td className="premium-col">Yes</td></tr>
            <tr><td>Booking Requests</td><td>Spam Calls</td><td className="premium-col">Serious Tenants Only</td></tr>
          </tbody>
        </table>
      </div>

      <h1 className="main-title">Your Room Is Invisible Right Now</h1>
      <p className="sub-text">Free listings get buried. Premium rooms get booked.</p>
      ...
      <div className="pricing-dual owner-price">
        {premiumOwnerPlans.map(plan => {
          ...
          const hierarchy = ["OWNER_TRIAL","OWNER_1M","OWNER_6M","OWNER_12M"];
          ...
          return (
            <div key={plan.code} className={`pricing-box ${isCurrent ? "current premium main" : "trial"}`}>
              ...
              {plan.code === "OWNER_1M" && <div className="popular-badge">Most Popular</div>}

              <h2>{plan.label}</h2>
              <p className="price">₹{plan.amount} <span>/ {plan.duration}</span></p>

              <ul>
                {premiumOwnerFeatures.map(f => <li key={f}>{f}</li>)}
              </ul>

              <RazorPayConfig
                amountToPay={plan.amount}
                value={isPremiumUser ? "Extend Plan" : status?.premium ? "Upgrade" : "Get Premium"}
              />
